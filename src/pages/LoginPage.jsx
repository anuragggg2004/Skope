...
const TESTIMONIALS = [
  {
    text: "I thought I was destined for engineering. Skope told me I had 64% and no coding experience. It showed me Actuarial Science. Now I'm studying at IISER.",
    name: "Ananya S.",
    tag: "IISER Pune"
  },
  {
    text: "My parents wanted IIT. Skope said 'that conversation is over with your percentile' and recommended DAIICT. Best decision of my life.",
    name: "Rohan M.",
    tag: "DAIICT Gandhinagar"
  },
  {
    text: "No one told me UCEED existed. Skope did. I gave it and got into IIT Bombay's Design program.",
    name: "Priya K.",
    tag: "IIT Bombay B.Des"
  }
]
...
export default function LoginPage() {
  ...
  const handleForgotSubmit = async (e) => {
    ...
    try {
      ...
      const msgs = { 'auth/user-not-found': 'No account with this email.', 'auth/invalid-email': 'Invalid email format.' }
  ...
  return (
    ...
    <>
      ...
      <div className="min-h-screen flex" style={{ background: '#080b14' }}>
        ...
        {/* ─── LEFT PANEL ────────────────────────────── */}
        <div className="hidden lg:flex flex-col justify-between w-[460px] shrink-0 relative overflow-hidden p-10"
          style={{ borderRight: '1px solid rgba(79,142,247,0.08)', background: 'linear-gradient(160deg, rgba(79,142,247,0.05) 0%, rgba(139,92,246,0.08) 50%, #080b14 100%)' }}>
          ...
          <div className="rounded-[16px] p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <p className="font-dm text-[13px] text-[rgba(240,242,255,0.6)] leading-[1.75] italic mb-4">
              "{t.text}"
            </p>
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="font-sora text-[12px] font-semibold text-white">{t.name}</div>
                <div className="font-dm text-[10px] text-[rgba(240,242,255,0.35)]">{t.tag}</div>
              </div>
            </div>
            <div className="flex gap-1.5 mt-4">
              {TESTIMONIALS.map((_, i) => (
                <button key={i} onClick={() => setActiveTestimonial(i)}
                  className={`h-1 rounded-full border-none cursor-pointer transition-all duration-300 ${i === activeTestimonial ? 'w-6 bg-[#4f8ef7]' : 'w-2 bg-[rgba(255,255,255,0.15)]'}`}
                />
              ))}
            </div>
          </div>
          ...
          <div className="w-full max-w-[400px] animate-fadeUp">
            ...
            <div className="mb-7">
              <h1 className="font-sora text-[28px] font-bold text-white tracking-[-0.5px] mb-1.5">
                ...
                {viewMode === 'phone' && 'Phone sign-in'}
                {viewMode === 'otp' && 'Verify your number'}
              </h1>
              <p className="font-dm text-[13px] text-[rgba(240,242,255,0.4)]">
                ...
                {viewMode === 'phone' && 'We\'ll send a 6-digit OTP to your number.'}
                {viewMode === 'otp' && `OTP sent to +91 ••••••${phone.slice(-4)}`}
              </p>
            </div>
            ...
            {viewMode === 'login' && (
              <>
                ...
                <button onClick={handleGoogleLogin} disabled={googleLoading}
                  ...
                  {googleLoading ? <div className="w-5 h-5 border-2 border-[#ccc] border-t-[#333] rounded-full animate-spin" /> : <IconGoogle />}
                ...
                <button onClick={handleGuestLogin} disabled={guestLoading}
                  ...
                  {guestLoading ? <div className="w-4 h-4 border-2 border-[rgba(240,242,255,0.3)] border-t-white rounded-full animate-spin" /> : <IconGuest />}
                ...
                <form onSubmit={handleEmailSubmit} className="space-y-4">
                  {isSignup && (
                    <div>
                      ...
                      <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="What should we call you?" className={inputCls} />
                    </div>
                  )}
                  <div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      {!isSignup && (
                        <button type="button" onClick={() => { setViewMode('forgot'); setError('') }}
                    ...
                    <div className="relative">
                      <input type={showPw ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                        placeholder={isSignup ? 'At least 6 characters' : 'Your password'} className={inputCls + ' pr-11'} />
                  ...
                  <button type="submit" disabled={loading} className={btnPrimary}>
                    {loading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <IconMail />}
                  ...
                  <form onSubmit={handleForgotSubmit} className="space-y-4">
                    <div>
                      ...
                      <input type="email" value={forgotEmail} onChange={e => setForgotEmail(e.target.value)}
                        placeholder="The email you signed up with" className={inputCls} autoFocus />
                    </div>
                    <button type="submit" disabled={forgotLoading} className={btnPrimary}>
                      {forgotLoading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <IconSend />}
                      {forgotLoading ? 'Sending...' : 'Send Reset Link'}
                    </button>
                  </form>
                )}
                <button type="button" onClick={() => { setViewMode('login'); setForgotSuccess(false); setError('') }}
